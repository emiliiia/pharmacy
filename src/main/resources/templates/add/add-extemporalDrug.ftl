...
<div class="mainGroundwork">
    ...
    <div class="titleComponentDIV">
        <h1 id="titleComponent">Add extemporal drug form</h1>
    </div>
    <div style="height: 1000px; width: 1000px" class="AddContainer">
        <div class="brand-logoDrug"></div>
        <div class="brand-title">Drug</div>
        <form name="extemporalDrug" action="endCreateForm" method="post">
            <div class="inputs">
                <div style="width: 450px">
                <label>NAME</label>
                <@spring.formInput "form.extemporalName", "placeholder='Drug name'" "text"/>
                <label>VOLUME</label>
                <@spring.formInput "form.volume", "step=0.1" "number"/>
                <label>USE</label>
                <div class="select">
                    <@spring.formSingleSelect "form.use", use "select"/>
                </div>
                <label>PRESCRIPTION</label>
                <@spring.formTextarea "form.prescript", "placeholder='Prescription...'" />
                <label>RELEASE FORM</label>
                <div class="select">
                    <@spring.formSingleSelect "form.releaseForm", extemporalReleaseForm "select"/>
                </div>
                <label>EXPIRATION</label>
                <@spring.formInput "form.expirationDate", "placeholder='5 days/months/years'" "text" />
                <label>TECHNOLOGY</label>
                <div class="select">
                    <@spring.formSingleSelect "form.technologyId", technologyBookList "select"/>
                </div>
                </div>
                <form action="http://localhost:8082/ui/v1/mongoDB/extemporal/drugs/endCreateForm">
                    <button type="submit">ADD NEW EXTEMPORAL DRUG</button>
                </form>


                <div style="height: 750px; width: 400px; margin-left: 490px; margin-top: -780px;">
                <label>COMPONENTS</label>
                <div>
                    <@spring.formMultiSelect "form.componentList", componentMap "multiply='true'" />
                </div>
                </div>
            </div>
        </form>
