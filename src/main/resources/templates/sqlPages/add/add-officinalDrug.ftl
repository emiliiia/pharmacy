...
<div class="mainGroundwork">
    ...
    <div class="titleComponentDIV">
        <h1 id="titleComponent">Add officinal drug form</h1>
    </div>
    <div style="height: 1150px" class="AddContainer">
        <div class="brand-logoOfficinal"></div>
        <div class="brand-title">Officinal Drug</div>
        <form name="officinalDrug" action="endCreateForm" method="post">
            <div class="inputs">
                <label>NAME</label>
                <@spring.formInput "form.drugName" "placeholder='Drug name'" "text" />
                <label>IN STOCK</label>
                <@spring.formInput "form.officinalInStock", "step=1" "number"/>
                <label>VOLUME</label>
                ...
                <div class="select">
                    <@spring.formSingleSelect "form.releaseForm", releaseForm "select"/>
                </div>
                <label>EXPIRATION</label>
                <@spring.formInput "form.officinalExpirationDate"  "placeholder='yyyy-mm-dd'"  "date" />
                <label>MANUFACTURER</label>
                <div class="select">
                    <@spring.formSingleSelect "form.manufacturerId", manufacturerList "select"/>
                </div>
                <label>PRICE</label>
                <@spring.formInput "form.price" "step=0.01" "number"/>
                <form action="http://localhost:8082/ui/v1/postgreeSQL/officinal/drugs/endCreateForm">
                    <button type="submit">ADD NEW DRUG</button>
                </form>
            </div>
        </form>
    </div>
